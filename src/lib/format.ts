...
export function roadDisplayName(r: RoadFeature): string {
	const p = r.properties;
	if (p.itinerary && p.name && p.itinerary !== p.name) {
		return p.itinerary;
	}
	return p.itinerary || p.name || UNNAMED;
}
...
export function roadNarrative(r: RoadFeature): string {
	...
	if (p.constructionPeriod) pieces.push(`Built in the ${p.constructionPeriod} period.`);
	return pieces.join(" ");
}
